    ...
    <div class="channelForm">
      <el-form ref="form" :model="formadd" :rules="ruleValidate" label-width="100px">
        <el-form-item label="权限名称" prop="name">
          <el-input v-model="formadd.name" placeholder="请输入权限名称" />
        </el-form-item>
        <el-form-item label="权限类型" prop="type">
          <el-select v-model="formadd.type" placeholder="请选择权限类型" style="width: 100%;">
...
<script>
import { createPermission } from '@/api/mypermission'
...
export default {
  ...
  methods: {
    ...
    createPermission() {
      ...
      this.loading = true
      createPermission(this.formadd).then(response => {
        this.$message({
          message: '创建成功！',
          type: 'success'
        })
